...
public class UserSpecification {
    private static final Map<String, List<String>> COUNTRY_ALIASES = Map.of(
            "GB", List.of("great britain", "britain", "england", "uk")
    );

    private static Set<String> resolveCountryCodes(String queryLower) {
        Set<String> codes = Locale.getISOCountries(Locale.IsoCountryCode.PART1_ALPHA2).stream()
                .filter(code -> Locale.of("", code).getDisplayCountry(Locale.ENGLISH)
    ...
    public static Specification<User> withFilters(UserFilterDTO filterDTO) {
        return (root, query, cb) -> {
            ...
            if (filterDTO.getQuery() != null && !filterDTO.getQuery().isBlank()) {
                ...
                orPredicates.add(cb.like(cb.lower(cb.concat(cb.concat(root.get("firstName"), " "), root.get("surname"))), queryPattern));
