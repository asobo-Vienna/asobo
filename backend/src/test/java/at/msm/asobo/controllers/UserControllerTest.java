...
public class UserControllerTest {
  ...
  void getUserById_returnsExpectedResult() throws Exception {
    ...
    mockMvc
        .perform(get("/api/users/id/{id}", expectedUser.getId()).accept(MediaType.APPLICATION_JSON))
        .andDo(print())
  ...
  void getUserById_whenUserNotFound_returns404() throws Exception {
    ...
    when(userService.getUserDTOById(any(UUID.class))).thenThrow(new UserNotFoundException(userId));

    mockMvc
        .perform(get("/api/users/id/{id}", userId).accept(MediaType.APPLICATION_JSON))
        .andDo(print())
  ...
  void getUserByUsername_returnsExpectedResult() throws Exception {
    ...
    UserPublicDTO expectedUser =
        new UserTestBuilder().withId(testId).withUsernameAndEmail("testuser").buildUserPublicDTO();
    ...
    mockMvc
        .perform(get("/api/users/{username}", "testuser").accept(MediaType.APPLICATION_JSON))
        .andDo(print())
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
        .andExpect(content().json(expectedJson));
  ...
  void getUserByUsername_whenUserNotFound_returns404() throws Exception {
    ...
    mockMvc
        .perform(get("/api/users/{username}", username).accept(MediaType.APPLICATION_JSON))
        .andDo(print())
        .andExpect(status().isNotFound());
